...
class EyeTrackingFeature:
    ...
    def __call__(self, frame):
        ...
        for rect in rects:
            ...
            joint_position_vector = np.concatenate((end_points_left, end_points_right))
            if len(self.position_buffer) > self.max_buffer_size:
                self.position_buffer.pop(0)
            self.position_buffer.append(joint_position_vector)

            # if len(position_buffer) > 1:
            std_feature = np.mean(np.std(np.array(self.position_buffer), axis=0))
            # print(std_feature)
            self.smoothed_std = self.smoothing * self.smoothed_std + (1-self.smoothing) * std_feature

        transformed_val = self.transform(self.smoothed_std)
        return transformed_val
...
#     #     joint_position_vector = np.concatenate((end_points_left, end_points_right))
#     #     if len(position_buffer) > max_buffer_size:
#     #         position_buffer.pop(0)
#     #     position_buffer.append(joint_position_vector)

#     #     if len(position_buffer) > 1:
#     #         std_feature = np.mean(np.std(np.array(position_buffer), axis=0))
#     #         print(std_feature)
